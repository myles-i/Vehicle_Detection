...
def single_img_features(img, color_space='RGB', spatial_size=(32, 32),
    ...
    if color_space != 'RGB':
        if color_space == 'HSV':
            feature_image = cv2.cvtColor(img, cv2.COLOR_RGB2HSV).astype(np.float32)/255
        elif color_space == 'LUV':
            feature_image = cv2.cvtColor(img, cv2.COLOR_RGB2LUV).astype(np.float32)/255
        elif color_space == 'HLS':
            feature_image = cv2.cvtColor(img, cv2.COLOR_RGB2HLS).astype(np.float32)/255
        elif color_space == 'YUV':
            feature_image = cv2.cvtColor(img, cv2.COLOR_RGB2YUV).astype(np.float32)/255
        elif color_space == 'YCrCb':
            feature_image = cv2.cvtColor(img, cv2.COLOR_RGB2YCrCb).astype(np.float32)/255
...
def add_heat(heatmap, bbox_list, alpha):
